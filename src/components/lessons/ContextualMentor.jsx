...
4. Respond in Russian using Markdown formatting.`;
  ...
  return (
    <div className="w-full lg:w-[350px] xl:w-[400px] border-l border-white/10 bg-[#0c0c0e] flex flex-col h-full shrink-0 relative overflow-hidden">
      {/* Header */}
      <div className="h-14 border-b border-white/10 flex items-center px-4 bg-[#09090b] shrink-0 justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-white tracking-tight">Умный Наставник</span>
        </div>
        {!isFree && (
      ...
      <div className="flex-1 flex flex-col relative overflow-hidden">
        ...
        {isFree && (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center p-6 text-center bg-[#0c0c0e]/60 backdrop-blur-md">
            ...
            <h3 className="text-lg font-bold text-white mb-2">Наставник по уроку</h3>
            ...
            <button 
              ...
              className="bg-white text-black px-6 py-3 rounded-xl font-bold text-xs uppercase tracking-wider hover:bg-zinc-200 transition-colors shadow-lg"
            >
              Прокачать тариф
            </button>
          </div>
        ...
        <div className={`flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar ${isFree ? 'opacity-30 pointer-events-none select-none filter blur-[3px]' : ''}`}>
            ...
            <div key={msg.id} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
              <div className={`max-w-[90%] rounded-2xl p-3 text-xs leading-relaxed ${
                msg.role === 'user'
                  ...
                  : 'bg-[#1C1C1E] border border-white/5 text-zinc-200 rounded-bl-none'
              }`}>
          ...
          {generating && (
            <div className="flex items-center gap-2 text-zinc-400 text-[10px] bg-[#1C1C1E] border border-white/5 w-fit rounded-xl px-3 py-2">
      ...
      {/* Input Form */}
      <form onSubmit={handleSendMessage} className={`p-3 border-t border-white/10 bg-[#09090b] flex gap-2 shrink-0 ${isFree ? 'opacity-30 pointer-events-none blur-[2px]' : ''}`}>
        <input
          type="text"
          ...
          className="flex-1 bg-[#1C1C1E] border border-white/5 rounded-xl px-3 py-2 text-xs text-white placeholder:text-zinc-500 focus:outline-none focus:border-indigo-500 transition-colors"
        />
        <button
          type="submit"
          disabled={generating || isFree || !input.trim()}
          className="w-8 h-8 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:bg-[#1C1C1E] disabled:text-zinc-600 text-white flex items-center justify-center transition-colors shrink-0"
        >
